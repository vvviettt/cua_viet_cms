...
export function buildFeedbackListUrl(path: string, opts: { page?: number; q?: string }): string {
...
export function FeedbackListPagination({
  basePath,
  currentPage,
  totalPages,
  totalItems,
  ...
  return (
    <div className="mt-8 flex flex-col items-center gap-4 border-t border-zinc-200 pt-6">
      <p className="text-center text-sm text-zinc-600">
        ...
        trong tổng{" "}
        <span className="font-medium text-zinc-800">{totalItems}</span> hồ sơ
      </p>
      {showNumberLinks ? (
        <Pagination>
          <PaginationContent className="flex-wrap justify-center gap-1">
            <PaginationItem>
              {currentPage <= 1 ? (
                ...
                <PaginationPrevious
                  href={buildFeedbackListUrl(basePath, { page: currentPage - 1, q })}
                  text="Trước"
                />
              )}
            </PaginationItem>
            {pages.map((entry, i) =>
              entry === "ellipsis" ? (
                ...
                <PaginationItem key={entry}>
                  <PaginationLink
                    href={buildFeedbackListUrl(basePath, { page: entry, q })}
