...
/** Trả về cho CKEditor / adapter tương thích Editor.js image. */
...
/**
 * Upload ảnh chèn vào rich text (multipart field `image`, hoặc JSON { url }).
 * Lưu buffer lên Supabase qua persistContentImageBuffer.
 */
export async function postRichTextEditorImageUpload(
  request: Request,
  session: SessionPayload,
  options?: { storageRelativePrefix?: string },
  ...
  if (ct.includes("multipart/form-data")) {
    ...
    try {
      const { url } = await persistContentImageBuffer({
        ...
        storageRelativePrefix: options?.storageRelativePrefix,
      });
  ...
  if (ct.includes("application/json")) {
    ...
    try {
      res = await fetch(safeUrl, {
        redirect: "follow",
        headers: { "User-Agent": "CmsRichTextContentImage/1.0" },
